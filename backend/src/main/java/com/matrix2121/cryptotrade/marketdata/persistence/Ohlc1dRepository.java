...
@Repository
public interface Ohlc1dRepository extends JpaRepository<Ohlc1d, OhlcBucketId> {

    Optional<Ohlc1d> findFirstBySymbolAndBucketLessThanEqualOrderByBucketDesc(
            String symbol, Instant bucket);

    Optional<Ohlc1d> findFirstBySymbolOrderByBucketDesc(String symbol);

    @Query("SELECT MAX(o.bucket) FROM Ohlc1d o WHERE o.symbol = :symbol")
    Instant findMaxBucketBySymbol(@Param("symbol") String symbol);

    @Query("SELECT MIN(o.bucket) FROM Ohlc1d o WHERE o.symbol = :symbol")
