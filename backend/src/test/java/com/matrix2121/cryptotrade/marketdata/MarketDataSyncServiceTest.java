...
class MarketDataSyncServiceTest {
    ...
    @Mock
    private OhlcChartRepository ohlcChartRepository;
    @Mock
    ...
    @Mock
    private ContinuousAggregateRefreshService caggRefreshService;
    ...
    void syncAll_skipsKrakenFetchWhenNoGaps() {
        ...
        long eightHourWindowStart = now - 40L * 24 * 60 * 60 * 1000L;
        when(ohlcChartRepository.findBucketEpochMsBetween(eq("ohlc_8h"), anyString(), anyLong(), anyLong()))
                .thenReturn(generateTimestamps(eightHourWindowStart, now, 8L * 60 * 60 * 1000L));
    ...
    @Test
    void syncAll_backfillsHourlyWhenEightHourChartHasGaps() {
        long now = System.currentTimeMillis();
        long windowStart = now - 40L * 24 * 60 * 60 * 1000L;
        long gapBucket = alignDown(windowStart + 8L * 60 * 60 * 1000L, 8L * 60 * 60 * 1000L);

        when(ohlc1mRepository.findBucketEpochMsBetween(anyString(), anyLong(), anyLong()))
                .thenReturn(generateTimestamps(OhlcIntervalPolicy.windowStartMs("1m", now), now, 60_000L));
        when(ohlcChartRepository.findBucketEpochMsBetween(eq("ohlc_8h"), eq("BTC/USD"), anyLong(), anyLong()))
                .thenReturn(List.of(gapBucket + 8L * 60 * 60 * 1000L));

        long hourlyTs = gapBucket + 60_000L;
        OhlcDto hourly = new OhlcDto(
                hourlyTs,
                BigDecimal.ONE,
                BigDecimal.valueOf(2),
                BigDecimal.ONE,
                BigDecimal.valueOf(2),
                BigDecimal.TEN);
        when(krakenOhlcClient.fetchOhlcSince(eq("BTC/USD"), eq(60), anyLong()))
        ...
        verify(caggRefreshService).refreshOneMinuteDerived();
    }
